...
namespace SharpDevelopIDEHost
{
    ...
    /// is copied to separate folder in isolated storage. ITM loads addin .dll from this created directory.
    /// 
	...
	static class IsolatedStorageService
	{
		...
		public static string CopyFileToStorage(string filename)
		{
		...
		public static void ClearStorage()
		{
			try
			{
                //Waiting need to be sure files are not being in use by ITM                
			...
			{
				//Skip error
				//MessageBox.Show(e.Message, "Error while clear isolated storage");
			}
		}
	}
}
